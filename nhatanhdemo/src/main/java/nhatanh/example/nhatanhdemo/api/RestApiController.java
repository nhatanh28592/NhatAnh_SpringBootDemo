...
import nhatanh.example.nhatanhdemo.form.ErrorInfo;
import nhatanh.example.nhatanhdemo.form.SearchRequest;
import nhatanh.example.nhatanhdemo.form.SuccessInfo;
...
public class RestApiController {
	...
	public Object updateTodo(@Valid @RequestBody Todo todoForm) {
		...
		//Check id exist
		Todo todo = todoService.findById(todoForm.getId());
		if(todo == null) {
	...
	public Object deleteTodo(@PathVariable(value = "id") long id) {
	    ...
	    if(todo == null) {
	        return ResponseEntity.ok(new ErrorInfo("/todo/delete", "Id", "Id not exist!")); 
	    }

	    todoService.delete(todo);
	    return ResponseEntity.ok(new SuccessInfo("/todo/delete", "Delete complete", todo.getId())); 
	}
	...
	private static List<ErrorInfo> checkTodo(Todo todo, String path){
		...
		if (todo.getWorkName() == null || todo.getWorkName().equals("")) {
			errors.add(new ErrorInfo(path, "workName", "Work name must not null and not blank!"));
		}
		return errors;
	}
}
